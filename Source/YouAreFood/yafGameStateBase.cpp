...
void AYafGameStateBase::GetSidePieceDecorations(UStaticMesh*& MeshToDisplay, TSubclassOf<AActor>& ActorToDisplay)
{
	MeshToDisplay = nullptr;
  	ActorToDisplay = nullptr;
 	
 	if (SidePieceDecoration > -1)
 	{		
 		if (EdgeActorBPs.Num() > 0)
 		{
 		...
 		if (EdgeActorBPs.Num() > 0)
 		{
 			ActorToDisplay = EdgeActorBPs[SidePieceDecoration];
 		}
 	}
}
